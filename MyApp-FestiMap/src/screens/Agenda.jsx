...
const COLORS = {
  accent: '#ffb800', // Oro
  violet: '#8b5cf6', // Violeta Eléctrico
  ink: '#0f172a',    // Fondo profundo
  white: '#ffffff',
...
export default function Agenda({ navigation }) {
  const { agenda, planes, quitarEvento, eliminarPlan } = useAgenda();
  const [tab, setTab] = useState('eventos'); // 'eventos' o 'planes'
            ...
            planes.length === 0 ? renderEmptyState('planes') : (
              planes.map((plan, index) => (
                <TouchableOpacity 
                  ...
                  onPress={() => navigation.navigate('PlanTab', { planId: plan.idPlan })}
                  activeOpacity={0.9}
                >
                  ...
                  <View style={styles.planInfo}>
                    <View style={styles.planHeaderRow}>
                      <Text style={styles.planTitle} numberOfLines={1}>{plan.nombre.toUpperCase()}</Text>
                      <TouchableOpacity onPress={() => eliminarPlan(plan.idPlan)}>
                    ...
                    <View style={styles.planFooter}>
                      <Text style={styles.planDate}>📅 {plan.creado}</Text>
                      <Text style={styles.planStops}>📍 {Object.keys(plan.itinerario || {}).length} paradas</Text>
                    </View>
                  </View>
                </TouchableOpacity>
              ))
